...
<div id="body_wrap">
  ...
  <table>
		...
		$query = $this->db->get_where('examinationx24', array('searchID' => $patient_id,'examinationID'=>$exam_id));
		...
		$row['symptom'] = $query->row()->symptom;?>
		    ...
		    <tr>
		      <td colspan="3"><p>
		        ...
		        <br/>
		        <textarea id="symptom" name="symptom" class="element symptom medium"value="" readonly><?php echo $row['symptom']; ?></textarea>
		        </p>
		        <p>
				...
				if($this->flexi_auth->get_user_group() == 'Operator' )
				{
				$this->db->select('assignedDoctor');
				$query = $this->db->get_where('operatory28', array('userID' => $operatorId));
				$doc['id'] = $query->row()->assignedDoctor;
				$this->db->select('docName');
				$query = $this->db->get_where('doctorx23', array('docID'=>$doc['id']));
				$doc['name'] =  $query->row()->docName;
				?>
                <input id="doctorList_" name="doctorList_"  type="text"  maxlength="255" value="<?php echo '[Doc ID : '.  ($doc['id']+100). '] [Name  : '.$doc['name'].']';?>" readonly style="width:60%"/>
                <input type="hidden" id="doctorList_1" name="doctorList_1" value="<?php echo  $doc['id'] ?>"/>
                <?php }
                else if($this->flexi_auth->get_user_group() == 'Master Admin' )
				{?>
		          <select id="doctorList" name="doctorList">
		            <?php 
				 
				  $query = $this->db->get_where('examinationx24', array('searchID' => $patient_id,'examinationID'=>$exam_id));	
				  $patient['doc'] = $query->row()->docID;
				  $this->db->select('docID, docName');
		  		  $query = $this->db->get('doctorx23');
				  foreach($query->result() as $row){ 
				  $doc['id'] =  $row->docID;
				  $doc['name'] =  $row->docName;
				  if( $patient['doc'] == $doc['id'])
				  {
				  ?>
                  
                    <option value="<?php echo  $doc['id'] ?>" selected><?php echo '[Doc ID : '.  ($doc['id']+100). '] [Name  : '.$doc['name'].']';?></option>
                    <?php
				  }
	...
	<?php }					 }} ?>
                    ...
                    <p> <input id="diagSubmit" class="link_button_b small" type="submit" name="diagSubmit" value="Submit" />  </p></td>
...
<form id="genPrimary" method="post" action="<?php echo $base_url;?>bmpt/load_report" target="_blank">
        <div align="center">
         ...
         <input type="hidden" id="henabled" name="henabled" value=1 />
    <input type="submit" id="prescription_primary" class="link_button_b medium" value="Submit Report" />
         <br/>
        </div>
        </form>
