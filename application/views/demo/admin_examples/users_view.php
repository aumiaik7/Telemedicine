...
<div id="body_wrap">
	...
	<div class="content_wrap intro_bg">
		<div class="content clearfix">
			<div class="col100">
				<h1>Admin: <?php echo $page_title;?></h2>
			
			<?php if (isset($status) && $status == 'failed_login_users') { ?>
				<p class="a">This page demonstrates a database query to display all user accounts that have a high number of failed login attempts since the users last successful login. Such data could be used to highlight potential brute force hacking attempts on user accounts.</p>
			<?php } else { ?>
				<?php if (isset($status) && $status == 'active_users') { ?>
					<p class="a">This page demonstrates a database query to display all users that have activated their account since registration.</p>
				<?php } else { ?>
					<p class="a">This page demonstrates a database query to display all users that have not activated their account since registration.</p>
				<?php } ?>
			<?php } ?>
			</div>		
		</div>
