...
class Demo_auth_admin_model extends CI_Model {
		...
		if ($this->flexi_auth->is_privileged('Delete Users')) 
		{
			if ($delete_users = $this->input->post('delete_user'))
			{
				foreach($delete_users as $user_id => $delete)
				{
					...
					$this->management->delete_user($data);
				}
			}
		}
	...
	function update_user_account($user_id)
	{
		...
		if ($this->form_validation->run())
		{
			...
			if($this->input->post('update_group') == 5)
			{
				$data = array('id'=> $user_id,  'fname'=>  $this->input->post('update_first_name'),  'lname'=>  $this->input->post('update_last_name'));
				
				$this->load->model('management');
				$this->management->create_doctor($data);
				//redirect('auth_lite/index');
			}		
			else if($this->input->post('update_group') == 4)
			{
				$data = array('id'=> $user_id,  'fname'=>  $this->input->post('update_first_name'),  'lname'=>  $this->input->post('update_last_name'));
				
				$this->load->model('management');
				$this->management->create_operator($data);
				//redirect('auth_lite/index');
			}		
			else
			{
