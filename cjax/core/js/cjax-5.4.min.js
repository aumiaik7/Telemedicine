...
switch(_raw_fn){case'confirm':if(my_fn(data.a)){console.log(data.b);if(typeof data.b=='function'){data.b();}}
...
if(typeof value=='object'){for(x in value){if(typeof value[x]=='object'){for(item in value[x]){element[x][item]=value[x][item];}}else{element[x]=value[x];}}}else{switch(element.nodeName){default:console.log(element);element.value=value;break;case'IMG':element.src=value;break;case'A':var href=element.href;if(/^https?/.test(href)){element.href=value;element.innerHTML=value;}else{element.innerHTML=value;}
break;case'LABEL':case'BUTTON':case'DIV':case'SPAN':element.innerHTML=value;break;case'INPUT':{switch(element.type){case'button':case'radio':case'text':case'select-one':case'textarea':case'hidden':case'password':element.value=value;break;case'checkbox':element.checked=parseInt(value)?true:false;}}
break;}}};this.remove=function(buffer){var element=CJAX.is_element(buffer);if(element){element.parentNode.removeChild(element);}};this.util=function()
...
return xml_arr;},strrpos:function(haystack,needle,offset){var i=haystack.lastIndexOf(needle,offset);return i>=0?i:false;},encode:function(str){str=escape(str);str=str.replace('+','%2B');str=str.replace('%20','+');str=str.replace('*','%2A');str=str.replace('/','%2F');str=str.replace('@','%40');str=str.replace('(','%28');str=str.replace('!','%21');str=str.replace(')','%29');return str;},injectXML:function(buffer,xml){buffer=buffer.replace(/<\/out>/gi,xml+'</out>');return buffer;},replace_encode:function(buffer,unencoded)
...
var script;var name='cjax-5.4.js';var src;script=CJAX.$('cjax_lib');if(script){src=script.src;var f=src.replace(/cjax-.+$/,'');CJAX.uri=script.src;}else{var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){script=scripts[i];src=script.src;if(CJAX.util.get.basename(src).indexOf('cjax-')!=-1){var f=src.replace(/cjax-.+$/,'');CJAX.uri=src;return f;}}}
...
var allow_input=CJAX.xml('allow_input',buffer);var _selected=CJAX.xml('selected',buffer);var options=CJAX.util.json(CJAX.decode(CJAX.xml('options',buffer)));var options_count=CJAX.util.count(options);if(element.type=='select-one'){element.style.display='inline';if(options_count){element.options.length=0;for(x in options){addOption(element,options[x],x);if(options_count==x){break;}}}else{if(allow_input){make_input(element);}else{element.style.display='none';}}
...
console.log(obj);var x=0;for(var i in options){x++;addOption(obj,options[i],i);if(options_count==x){break;}}
...
CJAX.lib.overlayOptions(options);CJAX.$('cjax_overlay').style.display='block';message_options={top:options.top,message:template,message_id:'cjax_message_overlay',success:function(){CJAX.lib.overlayCallback(content,callback,options);}};for(x in options){message_options[x]=options[x];}
...
CJAX.get(url,function(response){CJAX.lib.overlayCallback(response,callback,options);});}};for(x in options){message_options[x]=options[x];}
...
return element;},pharseFunction:function(buffer){if(typeof buffer=='object'){return buffer;}
if(typeof buffer=='string'&&buffer.substr(0,'function('.length)=='function('){try{fn=eval('('+buffer+')');}catch(e){console.log('Function Error:\n',e,'\n\n',buffer);}
...
return;}
var events=CJAX.util.json(CJAX.xml('events',buffer));var event=CJAX.xml('event',buffer);for(x in events){buffer=events[x];CJAX.set.event(element,buffer.event?buffer.event:event,buffer.xml,x);}};this.is_cjax=function(buffer){if(typeof buffer!='string'){return;}
...
{return CJAX.xml('uniqid',buffer);};this.set=function(){return{title:function(title){document.title=title;},event:function(element,_event,method,cache_id){var use_fns=false;if(typeof element=='string'){if(element.indexOf(':')!=-1){use_fns=true;fns=element.split(':');element=fns[0];element=CJAX.$(element);}else{element=CJAX.$(element);}}
if(CJAX.debug){console.log("set.even  for -..:",element);}
if(!element)return false;var element=CJAX.is_element(element);var f=method.toString();f=f.substr(0,f.indexOf('('));f=f.replace(/\s+$/,"");if(f=='function'||CJAX.lib.isFn(method)){if(typeof method=='function'){return CJAX._addEvent(element,_event,method,cache_id);}else{return CJAX._addEvent(element,_event,eval(method),cache_id);}}
var href;if(href=element.href){if(href.indexOf('#')==-1){element.onclick=function(){return false;};}else{element.href='javascript:void(0)';}}else{if(element.tagName=='IMG'){element.style.cursor='pointer';}
if(element.type&&(element.type=='checkbox'||element.type=='radio')){element.onclick=function(){return true;};}else{if(element.tagName=='LI'){element.onclick=function(){return false;};}else{element.onclick=function(){return false;};}}}
var _x=[];if(!cache_id){cache_id=new Date().getTime();}
var _stop=false;if(use_fns){for(var i=1;i<fns.length;i++){pub_fn=CJAX.lib.pharseFunction(fns[i]);x_fn=function(){props={exec:function(element){return pub_fn(element);},stop:function(){_stop=true;},start:function(){_stop=false;}}
...
if(plugin_name=CJAX.xml('is_plugin',method)){if(init=CJAX.inits[plugin_name]){plugin_fn=CJAX.lib.pharseFunction(init);plugin_fn();}
_fn=function(){if(!_stop){CJAX._extendPlugin(plugin_name,method,{element:element,event:_event,element_id:element.id,clear:function(){CJAX._EventCache.flushElement(element);}});}else{_stop=false;}};}else{_fn=function(data){_method=data.replace(/\n/g,"");if(!_stop){if(CJAX.util.isXML(_method)){if(!CJAX.is_cjax(_method)){_method="<cjax>"+_method+"</cjax>";}
CJAX._process(_method,'set.event',element);}else{eval(_method);}}else{_stop=false;}};}
if(_event=='onkeypress'){var key=CJAX.util.json(CJAX.xml('key',method));if(key){element.onkeypress=function(e){if(e.keyCode){element.keyCode=e.keyCode;}else{element.keyCode=e.charCode;}};new_fn=function(data){if(key[element.keyCode]){_fn(data);}};}else{new_fn=_fn;}}else{new_fn=_fn;}
_x[cache_id]=method;return CJAX._addEvent(element,_event,function(){new_fn(_x[cache_id]);},cache_id);},type:function(elem,new_type,verbose){if(!elem)return false;var elem=CJAX.is_element(elem,verbose);if(elem){elem.type=new_type;return true;}
return false;},'class':function(element,_class){element=CJAX.is_element(element,false);if(element){element.className=_class;}},center:function(obj,options){if(typeof options=='undefined')var options={};var element=CJAX.is_element(obj);if(options.width){prefix=parseInt(options.width);suffix=options.width.match(/(\%|px|em|pt|in|cm|mm|ex|pc)/)[0];if(!suffix){options.width=prefix+'px';}
switch(suffix)
...
file=CJAX.xml('file',buffer);file=CJAX.pBase+file;CJAX._plugins[plugin_method]=plugin_buffer;init=CJAX.xml('init',plugin_buffer);if(init){CJAX.inits[plugin_method]=init;}
waitfor=CJAX.xml('waitfor',plugin_buffer);if(waitfor){CJAX.found_extra_time+=250;_import=CJAX.importPlugin(file,function(){if(CJAX.debug){console.log('_import waitfor:',plugin_method,window[plugin_method]);}
...
CJAX._extendPlugin(plugin_method,plugin_buffer);continue;}else{if(id=CJAX.xml('waitFor',buffer)){if(cmd=CJAX.commands[id]){f=CJAX.xml('file',cmd).replace(/.*\//,'');if(!CJAX.loaded[f]){CJAX.waitingFor[CJAX.xml('file',cmd).replace(/.*\//,'')]=function(){CJAX.process(buffer,'process_all for '+method,method+' '+_id);};continue;}}}}
...
CJAX.is_loading=false;CJAX.timer=0;};this._extendPlugin=function(plugin_name,plugin_buffer,settings)
{if(waitfor=CJAX.xml('waitfor',plugin_buffer)){if(CJAX.debug){console.info(plugin_name,'waiting for',waitfor);}
return true;}
if(CJAX.debug){console.log('Loading:',CJAX.xml('is_plugin',plugin_buffer));}
file=CJAX.xml('file',plugin_buffer);file=CJAX.pBase+'/'+file;f=CJAX.xml('filename',plugin_buffer);_p=window[plugin_name];callbacks=CJAX.xml('callback',plugin_buffer);if(CJAX.debug&&callbacks){console.info('Plugin Buffer',plugin_buffer);console.info("Callbacks found for",plugin_name,callbacks);}
if(_p){if(plugin=CJAX.plugins[plugin_name]||CJAX.lib.isFn(_p)){if(CJAX.lib.isFn(plugin)){plugin=CJAX.extend(plugin,plugin_name,plugin_buffer,callbacks,settings);}else if(CJAX.lib.isFn(_p)){plugin=CJAX.extend(_p,plugin_name,plugin_buffer,callbacks,settings);}else{params=_p.params;_p.fn(params['a'],params['b'],params['c'],params['d'],params['e'],params['f']);}}}else{CJAX.lib.loadCallback(CJAX.loaded[f],function(){if(CJAX.lib.isFn(window[plugin_name])){plugin=CJAX.extend(window[plugin_name],plugin_name,plugin_buffer,callbacks,settings);}else{console.log('option ??');}},null,plugin_name);}};this._import=function(buffer,loop)
...
return f;};this.extend=function(plugin_fn,plugin_name,buffer,callbacks,settings)
...
var plugins_dir=__base__+'plugins/';var path=plugins_dir+file;var extra={};var file;var params={};if(buffer){file=CJAX.xml('file',buffer);if(_extra=CJAX.xml('extra',buffer)){extra=CJAX.util.json(_extra);}
var data=CJAX.decode(CJAX.xml('data',buffer));data=CJAX.util.json(data);for(x in data){if('object'==typeof data[x]){for(i in data[x]){data[x][i]=CJAX.lib.pharseFunction(data[x][i]);}}else{data[x]=CJAX.lib.pharseFunction(data[x]);}}
params=data;}
...
{if(typeof file=='object'){if(file.files){var time=300;if(file.time){time=file.time;}
var files=file.files.split(',');var last_number=CJAX.util.count(files)-1;var last_file=files[last_file];if(file.callback){delete files[last_file];}
testFile=function(_file,cb){if(/^https?/.test(_file)){return CJAX.importFile(files[_file],cb);}else if(file.plugin){return CJAX.importFile(CJAX.pBase+file.plugin+'/'+files[_file],cb);}else{return CJAX.importFile(files[_file],cb);}};for(xfile in files){f=testFile(xfile);}
if(file.callback){testFile(last_number,setTimeout(function(){file.callback();},time));}
return true;}else{if(file.plugin){file=CJAX.importFile(CJAX.pBase+file.plugin+'/'+file.file,$callback);}else{file=file.file;}}}
...
wait=CJAX._wait(buffer);if(_wait){if(wait){seconds=wait;}else{var seconds=CJAX.xml('seconds',buffer);if(!seconds&&CJAX.timer){seconds=CJAX.timer;}}
...
if(CJAX[SUBFIX]){try{if(seconds){buffer=buffer.replace(/[\n\r]/gm,'\\n');setTimeout(PREFIX+CJAX.method+'("'+buffer+'")',seconds);}else{fn=CJAX[SUBFIX];fn(buffer);}}catch(_e){alert('#process unabled to load function#1: '+CJAX.method+'();  '+_e.message);}
...
var timeout=CJAX.xml('timeout',buffer);ms=CJAX.xml('ms',buffer);var time_reset=parseInt(CJAX.xml('time_reset',buffer));if(time_reset){CJAX.timer=0;}
...
FLAG_TIMEOUT_EXPAND=CJAX.xml('expand',buffer);var not_wait=CJAX.xml('no_wait',buffer);if(not_wait){CJAX.funtion_timer=0;}else{CJAX.funtion_timer=true;}
if(!timeout){return;}
CJAX.waiting=true;if(!CJAX.timer){CJAX.timer=timeout;}else{CJAX.timer=parseInt(CJAX.timer)+parseInt(timeout);}
...
if(url){handlerFormRequest();CJAX.loading();}}else{if(url){handlerFormRequest();CJAX.loading();}}};this.refresh=function()
...
try{CJAX.HTTP_REQUEST_INSTANCE.send(((is_post)?full_url:null));}catch(e){console.log(e);};};this.setHandler=function(name,callback_handler)
{if(CJAX.lib.isFn(callback_handler)){CJAX.handlers[name]=callback_handler;}else{console.log('Invalid Handler was assigned');}};this.ajaxVars=function(buffer)
{vars=CJAX.xml('vars',CJAX.decode(buffer));if(vars){CJAX.ajaxSettings.AjaxVars=vars;}};this.$=function(element_id,v){if(!element_id){return;}
...
if(typeof $mix_item=='undefined'){var $mix_item={};}
...
CJAX.IS_POST=true;return CJAX.call(options,options.success);};this.get=function($url,container,success_callback){var options={};if(success_callback&&!CJAX.lib.isFn(success_callback)){CJAX.ajaxSettings.dataType=success_callback;success_callback=null;}
options.url=$url;if(/^https?/.test(options.url)){options.crossdomain=true;}
...
CJAX.$('cjax_overlay').style.display='none';}
url=CJAX._pharseValues(url);if(CJAX.ajaxSettings.stop){CJAX.ajaxSettings.stop=false;return true;}
if(CJAX.ajaxSettings.cache){lower_url=url.toLowerCase();if(CJAX.cache_calls[lower_url]){CJAX.ajaxSettings.cache=false;if(CJAX.callback_success[url]){CJAX.callback_success[url](CJAX.cache_calls[url]);}
...
CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With','CJAX FRAMEW0RK 5.4');return full_url;};this._handlerRequestStatus=function(url,status,container)
...
CJAX.cache_calls[url.toLowerCase()]=response;if(CJAX.ajaxSettings.success&&typeof CJAX.ajaxSettings.success=='function'){CJAX.ajaxSettings.success(response);}
...
if(typeof seconds=='undefined'){var seconds=CJAX.default_timeout;}
options={message:"<data><div class='cjax_message cjax_message_type cjax_success'>"+message+"</div></data>",time:seconds};CJAX.message(options);};this.warning=function(message,seconds)
{if(typeof message=='undefined'){var message=CJAX.defaultMessages.invalid;}
if(typeof seconds=='undefined'){var seconds=CJAX.default_timeout;}
options={message:"<data><div class='cjax_message cjax_message_type cjax_warning'>"+message+"</div></data>",time:seconds};CJAX._message(options);};this.error=function(message,seconds)
{if(typeof message=='undefined'){var message=CJAX.defaultMessages.error;}
if(typeof seconds=='undefined'){var seconds=CJAX.default_timeout;}
options={message:"<data><div class='cjax_message cjax_message_type cjax_error'>"+message+"</div></data>",time:seconds};CJAX._message(options);};this._pharseValues=function(url)
{var v=url.match(/\|(.+)\|/gi);try{for(x in v){id=v[x].replace(/\|/g,'');if(id=='rand'){element=CJAX.$(id);if(!element){_value=new Date().getTime();}else{_value=element.value;}}else{var use_fns=false;if(id.indexOf(':')!=-1){use_fns=true;fns=id.split(':');id=fns[0];element=CJAX.$(id);}else{element=CJAX.$(id);}
if(!element){continue;}
...
if(use_fns){for(var i=1;i<fns.length;i++){fn=CJAX.lib.pharseFunction(fns[i]);if(!CJAX.lib.isFn(fn)&&CJAX.lib.isFn(_value[fns[i]])){_value=_value[fn]();}else{_fn=function(){props={exec:fn,stop:function(){CJAX.ajaxSettings.stop=true;}}
...
return;}
CJAX.set.event(window,'load',fn);};this.onStartEvents=function(){CJAX.importFile(__base__+'core/css/cjax.css');if(typeof jQuery!='undefined'){jQuery.ajaxSetup({complete:function(a,b){if(a){CJAX.process_all(a.responseText);}}});}};}
